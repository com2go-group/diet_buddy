...
The landing page (`landing/`) has the same texts as static pages, `privacy.html`, `terms.html`
and `delete-account.html`, generated by `npm run landing:legal`. Re-run it whenever the texts
change. Hosting the landing page is the simplest way to get the store URLs live.
